...
const parseGraphSchema = async (
  ...
  new Promise(async resolve => {
    const newSchemaEl = parsedScriptElems.find(elem => {
      if (elem['@graph'] !== undefined && Array.isArray(elem['@graph'])) {
        return elem['@graph'].some(
          graphItem => graphItem['timestamp'] !== undefined,
        );
      }
    ...
    if (newSchemaEl) {
      const newSchemaData = newSchemaEl['@graph']['timestamp'];
